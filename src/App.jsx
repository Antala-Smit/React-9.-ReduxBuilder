  ...
  return (
    <>
      <div className="container-fluid">
        <div className="row">
          <h2 className='bg-primary py-4 ps-5 text-light'><span className='noteicon pe-2'><PiNotePencilBold /></span>Google Keep</h2>
              ...
              record.map((val) => {
                return (
                  <div className="box  col-3 p-2 " key={val.id}>
                    <div className="box-1 shadow p-3">
                      <h6>{val.title}</h6>
                      <p>{val.note}</p>
                      <span className='deletebtn d-flex justify-content-end align-items-center'>
                        <i onClick={() => dispatch(deleteuser(val.id))}><RiDeleteBin6Line /></i>
                      </span>
                    </div>
                  </div>
                )
